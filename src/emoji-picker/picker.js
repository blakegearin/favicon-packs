...
function traverseAndSetupBindings (rootElement, elementsToBindings) {
  ...
  if (
    elementsToBindings.size === 1 &&
    (topLevelBindings = elementsToBindings.get(0))
  ) {
    ...
    return map(
      ...
      (emoji, i) => {
        return html`<button
          ...
          style="${emoji.unicode
            ? ''
            : `--custom-emoji-background: url(${JSON.stringify(emoji.url)})`}"
        >
          ${emoji.unicode ? unicodeWithSkin(emoji, state.currentSkinTone) : ''}
        </button>`
          ...
          ${map(
            ...
            (emojiWithCategory, i) => {
              return html`<div>
                ...
                <div
                  class="emoji-menu ${i !== 0 &&
                  !state.searchMode &&
                  state.currentGroup.id === -1
                    ? 'visibility-auto'
                    : ''}"
                  style="${`--num-rows: ${Math.ceil(
                    emojiWithCategory.emojis.length / state.numColumns
                  )}`}"
...
function createRoot (shadowRoot, props) {
  ...
  function checkZwjSupportAndUpdate (zwjEmojisToCheck) {
    const allSupported = checkZwjSupport(
      zwjEmojisToCheck,
      refs.baselineEmoji,
      emojiToDomNode
    )
...
const DEFAULT_DATA_SOURCE = '/emoji-picker/emojibase/data.json'
